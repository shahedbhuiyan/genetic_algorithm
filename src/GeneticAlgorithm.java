public class GeneticAlgorithm {
    ...
    private Population selectTournamentPopulation(Population population) {
        Population torunamentPopulation = new Population(TOURNAMENT_SELECTION_SIZE);
        for(int x = 0; x < TOURNAMENT_SELECTION_SIZE; x++){
            torunamentPopulation.getChromosomes()[x] = population.getChromosomes()[(int)(Math.random()*population.getChromosomes().length)];
        }
        torunamentPopulation.sortChromosomesByFitness();
        return torunamentPopulation;
    }
}